...
fun ApprovalCenterScreen(viewModel: ApprovalCenterViewModel = viewModel()) {
    ...
    Scaffold(containerColor = MaterialTheme.colorScheme.background) { padding ->
        LazyColumn(
            ...
            item {
                Column {
                    Text("Approvals", style = MaterialTheme.typography.headlineMedium, fontWeight = FontWeight.Black)
                }
            }
            ...
            item {
                Card(
                    ...
                    Row(Modifier.fillMaxWidth().padding(18.dp), verticalAlignment = Alignment.CenterVertically) {
                        ...
                        FilterChip(
                            selected = pending.isNotEmpty(),
                            onClick = {},
                            enabled = false,
                            label = { Text(pending.size.toString()) },
                        )
                    }
                }
            }
            ...
            item { SectionHeader(Icons.Default.Settings, "Persistent capability grants", "Grants can cover the entire workspace or selected workspace-relative paths. All grantable capabilities use an R3 approval-bypass ceiling and remain bounded by the selected workspace/path scope. Protected capabilities remain non-grantable.") }
                ...
                Card(shape = RoundedCornerShape(20.dp), colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surfaceContainerLow)) {
                    Row(Modifier.fillMaxWidth().padding(14.dp), verticalAlignment = Alignment.CenterVertically) {
                        Column(Modifier.weight(1f)) {
                            ...
                            Text("Approval bypass ceiling: R3", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                        }
    ...
    Card(shape = RoundedCornerShape(24.dp), colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surface)) {
        Column(Modifier.fillMaxWidth().padding(18.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
            ...
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                ...
                OutlinedButton(onClick = { onReject(action) }, modifier = Modifier.weight(1f)) {
