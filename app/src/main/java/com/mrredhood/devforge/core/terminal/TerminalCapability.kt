...
enum class TerminalExecutable(val binaryPath: String, val risk: RiskLevel) {
    ...
    DATE("/system/bin/date", RiskLevel.R1),
    DF("/system/bin/df", RiskLevel.R1),
    DU("/system/bin/du", RiskLevel.R1),
    STAT("/system/bin/stat", RiskLevel.R1),
    READLINK("/system/bin/readlink", RiskLevel.R1),
    REALPATH("/system/bin/realpath", RiskLevel.R1),
    BASENAME("/system/bin/basename", RiskLevel.R1),
    DIRNAME("/system/bin/dirname", RiskLevel.R1),
    ...
    DIFF("/system/bin/diff", RiskLevel.R1),
    SED("/system/bin/sed", RiskLevel.R2),
    MKDIR("/system/bin/mkdir", RiskLevel.R2),
    RMDIR("/system/bin/rmdir", RiskLevel.R2),
...
class TerminalCapability(
    ...
    private val githubTerminal = GitHubRemoteTerminal(GitHubRepositoryGateway(CredentialSecurityStore(context)))

    suspend fun prepareWorkspace(workspaceId: String, workspaceRoot: Uri): Result<Unit> =
        runCatching {
            if (githubStore.get(workspaceId) == null) {
                terminal.prepareWorkspace(workspaceId, workspaceRoot)
            }
        }
    ...
    suspend fun executeAuthorizedStreaming(
        workspaceId: String,
    ...
    suspend fun executeInteractiveShell(
        ...
        if (remote != null) {
            ...
            return try {
                val text = githubTerminal.execute(remote, workingDirectory, normalized)
                onOutput(text)
                TerminalExecution(
                    TerminalCommand(TerminalExecutable.PWD, workingDirectory = workingDirectory, timeoutMs = timeoutMs),
                ...
                TerminalExecution(
                    ...
                    1,
                    error.message ?: "Unable to read the GitHub workspace.",
                    (System.nanoTime() - startedAt) / 1_000_000L,
                )
            }
        }
    ...
    private suspend fun authorizeAndExecute(
        ...
        val action = actionRequest(workspaceId, command)
        if (!skipApproval && approvalId == null && DefaultPolicy.requiresApproval(action, permissionMode)) {
