...
object AICommandRegistry {
    val commands: List<AICommandDefinition> = listOf(
        ...
        AICommandDefinition("inspect", aliases = listOf("analyze"), description = "Inspect a workspace, repository, file, build, or Git state", usage = "/inspect <target>", instruction = "Inspect the requested target using the relevant DevForge read and analysis tools, then report evidence and findings."),
        AICommandDefinition("create", aliases = listOf("new"), description = "Create files or folders for a requested task", usage = "/create <request>", safety = AgentCommandSafety.SIDE_EFFECT, instruction = "Create the requested workspace files or folders using the appropriate DevForge creation tools, verify the results, and report exactly what was created."),
        AICommandDefinition("delete", aliases = listOf("remove"), description = "Delete a requested file or folder", usage = "/delete <path>", safety = AgentCommandSafety.SIDE_EFFECT, instruction = "Delete the requested workspace file or folder recursively when applicable. Use DevForge's approval-gated delete_path tool and verify the deletion."),
        AICommandDefinition("move", description = "Move a file or folder", usage = "/move <source> <destination>", safety = AgentCommandSafety.SIDE_EFFECT, instruction = "Move the requested workspace file or folder using the move_file or move_folders tools and verify the new path."),
        AICommandDefinition("rename", description = "Rename a file or folder", usage = "/rename <path> <new name>", safety = AgentCommandSafety.SIDE_EFFECT, instruction = "Rename the requested workspace file or folder using the rename_file or rename_folder tools and verify the result."),
        AICommandDefinition("logs", aliases = listOf("actions-logs"), description = "Inspect GitHub Actions or build logs", usage = "/logs <run or build>", instruction = "Inspect the relevant current GitHub Actions/build run with the GitHub workflow tools, recover from transient log retrieval failures, and report verified failures."),
        AICommandDefinition("preview", aliases = listOf("web-preview"), description = "Prepare a Web Live Preview for a workspace file", usage = "/preview <file>", instruction = "Inspect the requested web file and its workspace-relative assets, then prepare the changes or verification needed for Web Live Preview."),
        AICommandDefinition("verify", aliases = listOf("check"), description = "Verify a requested change or result", usage = "/verify <target>", instruction = "Run the relevant read-only checks, tests, lint, build, or workspace inspection needed to verify the requested result. Report only evidence-backed outcomes."),
        AICommandDefinition("github", aliases = listOf("remote"), description = "Perform a GitHub repository operation", usage = "/github <request>", safety = AgentCommandSafety.SIDE_EFFECT, instruction = "Use the appropriate GitHub tools for repository files, branches, issues, pull requests, Actions, artifacts, and commits while respecting approval gates."),
        AICommandDefinition("media", aliases = listOf("audio", "video"), description = "Inspect or extract media information", usage = "/media <request>", instruction = "Use the appropriate media analysis tools to inspect video/audio metadata, duration, streams, frames, artwork, or checksums."),
    )
