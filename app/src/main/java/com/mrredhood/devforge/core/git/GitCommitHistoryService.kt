...
class GitCommitHistoryService(
    ...
    suspend fun load(repository: GitRepositoryState, maxCommits: Int = MAX_COMMITS): GitHistorySnapshot {
        ...
        while (!currentId.isNullOrBlank() && traversed < maxCommits) {
            ...
            val changedFiles = changedFilesForCommit(reader, commit.treeId, commit.parents.firstOrNull(), MAX_CHANGED_FILES)
        ...
        return GitHistorySnapshot(
            commits = commits,
            truncated = !currentId.isNullOrBlank(),
        )
    }
    ...
    suspend fun changedFiles(repository: GitRepositoryState, commitId: String, maxFiles: Int = MAX_CHANGED_FILES): List<GitFileHistoryEntry> {
        ...
        return changedFilesForCommit(reader, commit.treeId, commit.parents.firstOrNull(), maxFiles)
    }

    suspend fun fileHistory(
        repository: GitRepositoryState,
        path: String,
        maxCommits: Int = MAX_FILE_HISTORY_COMMITS,
    ...
    private fun changedFilesForCommit(reader: GitObjectReader, treeId: String, parentId: String?, maxFiles: Int): List<GitFileHistoryEntry> {
        val currentTree = flattenTree(reader, treeId, maxFiles)
        val parentTree = parentId?.let { parent -> readCommit(reader, parent)?.let { flattenTree(reader, it.treeId, maxFiles) } }.orEmpty()
        val paths = (currentTree.keys + parentTree.keys).toSortedSet(String.CASE_INSENSITIVE_ORDER)
        val commit = readCommit(reader, "")
    ...
    private fun readCommit(reader: GitObjectReader, id: String): ParsedCommit? {
        ...
        return ParsedCommit(
            ...
            parents = parents,
            author = author,
            authoredAtEpochMs = authoredAt,
            subject = body.lineSequence().firstOrNull().orEmpty().trim().ifBlank { "(no commit message)" },
        )
    }

    private fun parseAuthor(header: String): Pair<String, Long?> {
        val match = Regex("^(.*) <([^>]+)> (-?\\d+) [+-]\\d{4}$").find(header) ?: return header to null
        val epochMs = match.groupValues[3].toLongOrNull()?.times(1000L)
        return match.groupValues[1] to epochMs
    }
    ...
    private fun walkTree(reader: GitObjectReader, treeId: String, prefix: String, result: MutableMap<String, String>, depth: Int, maxFiles: Int) {
        ...
        when (val tree = reader.readTree(treeId, MAX_TREE_ENTRIES)) {
            ...
            is GitObjectResultWithEntries.Success -> tree.entries.forEach { entry ->
                ...
                if (entry.mode.startsWith("04")) walkTree(reader, entry.objectId, path, result, depth + 1, maxFiles)
...
data class GitHistorySnapshot(
    val commits: List<GitCommitHistoryEntry>,
