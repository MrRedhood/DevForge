...
class AgentPatchPreviewService(
    ...
    suspend fun preview(approval: ApprovalEntity): AgentPatchPreview = withContext(Dispatchers.IO) {
        ...
        val root = if (isRemote) null else documentUri(Uri.parse(workspace.treeUri))
        val directPathExists = root?.let { exists(it, rawPath) } ?: false
        ...
        val remoteResult = if (remote != null) {
            githubGateway.readFile(remote.owner, remote.repository, path, remote.branch)
        } else null
        val (existed, before) = if (remote != null) {
            when (val result = remoteResult) {
                is GitHubFileResult.Success -> true to result.content
                is GitHubFileResult.Failure -> {
                    if (isRemoteNotFound(result.message)) {
                        false to ""
                    } else {
                        throw IllegalStateException(
                            "Unable to inspect GitHub patch target: ${result.message.take(240)}",
                        )
                    }
                }
                null -> throw IllegalStateException("GitHub workspace patch target could not be resolved.")
            }
        } else {
            val localRoot = root ?: throw IllegalStateException("Local workspace root is unavailable.")
            exists(localRoot, path) to readExisting(localRoot, path)
        }
    ...
    private fun isRemoteNotFound(message: String): Boolean {
