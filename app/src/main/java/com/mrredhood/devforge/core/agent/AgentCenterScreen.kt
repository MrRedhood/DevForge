    ...
    LazyColumn(
                ...
                Column(
                    ...
                    OutlinedButton(
                        ...
                        Column(
                            ...
                            Text(
                                "Tools: workspace · file edits · web · agent coordination",
        ...
        if (viewModel.tasks.isEmpty()) {
            ...
            items(viewModel.tasks, key = { it.taskId }) { taskEntity ->
                AgentTaskCard(taskEntity, viewModel, viewModel.handoffs)
            }
        }
    }
...
private fun AgentTaskCard(
    ...
    viewModel: AgentCenterViewModel,
    handoffs: List<com.mrredhood.devforge.core.storage.AgentHandoffEntity>,
) {
    ...
    Card(
        ...
        Column(
            ...
            val graph = AgentExecutionGraphBuilder.build(task, handoffs)
            Card(
                colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surfaceContainerLowest),
            ) {
                Column(Modifier.fillMaxWidth().padding(10.dp), verticalArrangement = Arrangement.spacedBy(3.dp)) {
                    Text(
                        "Execution graph · " + graph.nodes.size + " nodes · " + graph.edges.size + " edges",
                        style = MaterialTheme.typography.labelMedium,
                        fontWeight = FontWeight.SemiBold,
                    )
                    Text(
                        graph.nodes.take(6).joinToString(" → ") { it.title.take(32) },
                        style = MaterialTheme.typography.bodySmall,
                        color = MaterialTheme.colorScheme.onSurfaceVariant,
                        maxLines = 2,
                    )
            ...
            } else if (task.modelProviderId != null && task.modelId != null) {
                OutlinedButton(
                    onClick = { viewModel.replay(task.taskId) },
                    modifier = Modifier.fillMaxWidth(),
                ) { Text("Replay task") }
            }
        }
