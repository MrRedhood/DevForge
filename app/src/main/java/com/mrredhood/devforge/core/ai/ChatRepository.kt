...
class ChatRepository(
    ...
    suspend fun addMessage(
        ...
        val now = System.currentTimeMillis()
        val createdAt = maxOf(now, (messages.latestCreatedAt(sessionId) ?: Long.MIN_VALUE) + 1L)
        messages.insert(
            ChatMessageEntity(
    ...
    suspend fun getMessage(messageId: String): ChatMessageEntity? = messages.getById(messageId)

    suspend fun editMessage(messageId: String, content: String) {
        val message = messages.getById(messageId) ?: error("Chat message no longer exists.")
        require(message.role == "user") { "Only user messages can be edited." }
        require(content.length <= MAX_MESSAGE_CHARS) { "Chat message is too large." }
        val durableContent = SecretRedactor.redact(content, MAX_MESSAGE_CHARS)
        messages.updateContent(messageId, durableContent, System.currentTimeMillis())
    }

    suspend fun deleteMessagesAfter(sessionId: String, createdAtEpochMs: Long): Int =
        messages.deleteAfter(sessionId, createdAtEpochMs)
