...
class AgentToolGateway(
    ...
    suspend fun execute(context: AgentToolContext, request: AgentToolRequest): AgentToolResult {
        ...
        val action = actionRequest(request, tool.definition)
        ...
        if (needsApproval) {
            ...
            audit(
                ...
                metadata = JSONObjectLike.metadata("approvalId" to approvalId),
            )
    ...
    suspend fun executeApproved(
        ...
        if (approval.workspaceId != request.workspaceId || approval.actionId != action.actionId || approval.parametersHash != action.parametersHash) {
            return AgentToolResult.Failure("Approval '$approvalId' does not match the current tool request.")
        }
    ...
    private fun actionRequest(request: AgentToolRequest, definition: AgentToolDefinition): ActionRequest {
        ...
        return ActionRequest(
            ...
            parametersHash = hash(request),
        )
    }

    private fun hash(request: AgentToolRequest): String {
    ...
    private fun approvalPayload(request: AgentToolRequest): String =
        "{\"tool\":\"${request.toolId.wireName}\",\"workspaceId\":\"${escape(request.workspaceId)}\",\"taskId\":\"${escape(request.taskId)}\",\"stepIndex\":${request.stepIndex},\"arguments\":${request.argumentsJson}}"
