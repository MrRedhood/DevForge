...
class AgentCenterViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun replay(taskId: String) {
        val original = tasks.firstOrNull { it.taskId == taskId } ?: run {
            message = "Agent task is no longer available."
            return
        }
        val provider = AIProvider.entries.firstOrNull { it.id == original.modelProviderId } ?: run {
            message = "The original agent provider is no longer configured."
            return
        }
        val modelId = original.modelId?.trim().orEmpty()
        if (modelId.isBlank()) {
            message = "The original agent model is unavailable."
            return
        }
        assigning = true
        message = "Replaying " + original.title + "…"
        viewModelScope.launch(Dispatchers.IO) {
            try {
                val newTaskId = agentRuntime.assign(
                    AgentAssignment(
                        workspaceId = original.workspaceId,
                        title = original.title.take(200) + " · replay",
                        instruction = original.instruction.take(60_000),
                        model = AgentModelBinding(
                            provider,
                            modelId,
                            original.modelName?.ifBlank { modelId } ?: modelId,
                        ),
                        pathScope = WorkspacePathScope(),
                        access = runCatching {
                            AgentTaskPlanCodec.decode(original.payload).access
                        }.getOrDefault(AgentAccess.CODING_DEFAULT),
                    ),
                )
                launch(Dispatchers.Main.immediate) {
                    assigning = false
                    message = "Replay started: " + newTaskId.take(8)
                }
            } catch (cancelled: CancellationException) {
                throw cancelled
            } catch (error: Throwable) {
                launch(Dispatchers.Main.immediate) {
                    assigning = false
                    message = error.message ?: "Unable to replay the agent task."
                }
            }
        }
    }
