...
internal class GitObjectReader(
    ...
    fun read(sha: String, maxBytes: Int = MAX_OBJECT_BYTES): GitObjectResult = runCatching {
        ...
        if (!SHA_PATTERN.matches(normalized)) return@runCatching GitObjectResult.Unavailable("The requested Git object ID is invalid.")
        val objects = findDirectChild(gitDirectory, "objects") ?: return@runCatching GitObjectResult.Unavailable("The repository object directory is unavailable.")
        val prefix = findDirectChild(objects, normalized.take(2)) ?: return@runCatching GitObjectResult.Unavailable("Git object $normalized is not available as a loose object.")
        val objectUri = findDirectChild(prefix, normalized.drop(2)) ?: return@runCatching GitObjectResult.Unavailable("Git object $normalized is not available as a loose object.")
        val inflated = inflate(objectUri, maxBytes) ?: return@runCatching GitObjectResult.Unavailable("Git object $normalized could not be decompressed within the safety limit.")
        val headerEnd = inflated.indexOf(0)
        if (headerEnd <= 0 || headerEnd >= inflated.size) return@runCatching GitObjectResult.Unavailable("Git object $normalized has an invalid object header.")
        val header = String(inflated, 0, headerEnd, Charsets.UTF_8)
        val separator = header.indexOf(' ')
        if (separator <= 0 || separator == header.lastIndex) return@runCatching GitObjectResult.Unavailable("Git object $normalized has an invalid object header.")
        val type = header.substring(0, separator)
        val declaredSize = header.substring(separator + 1).toLongOrNull() ?: return@runCatching GitObjectResult.Unavailable("Git object $normalized has an invalid size header.")
        val content = inflated.copyOfRange(headerEnd + 1, inflated.size)
        if (declaredSize != content.size.toLong()) return@runCatching GitObjectResult.Unavailable("Git object $normalized is truncated or malformed.")
