...
internal class GitObjectReader(
    ...
    fun read(sha: String, maxBytes: Int = MAX_OBJECT_BYTES): GitObjectResult = runCatching {
        val normalized = sha.lowercase()
        if (!SHA_PATTERN.matches(normalized)) {
            return@runCatching GitObjectResult.Unavailable("The requested Git object ID is invalid.")
        }
        ...
        val prefix = findDirectChild(objects, normalized.take(2))
            ?: return@runCatching GitObjectResult.Unavailable("Git object $normalized is not available as a loose object.")
        val objectUri = findDirectChild(prefix, normalized.drop(2))
            ?: return@runCatching GitObjectResult.Unavailable("Git object $normalized is not available as a loose object.")

        val inflated = inflate(objectUri, maxBytes)
            ?: return@runCatching GitObjectResult.Unavailable("Git object $normalized could not be decompressed within the safety limit.")
        val headerEnd = inflated.indexOf(0)
        if (headerEnd <= 0 || headerEnd >= inflated.size) {
            return@runCatching GitObjectResult.Unavailable("Git object $normalized has an invalid object header.")
        }
        ...
        val declaredSize = header.substring(separator + 1).toLongOrNull()
            ?: return@runCatching GitObjectResult.Unavailable("Git object $normalized has an invalid size header.")
        val content = inflated.copyOfRange(headerEnd + 1, inflated.size)
        if (declaredSize != content.size.toLong()) {
            return@runCatching GitObjectResult.Unavailable("Git object $normalized is truncated or malformed.")
        }
        GitObjectResult.Success(type, content)
    }.getOrElse { error -> GitObjectResult.Unavailable(error.message ?: "Git object could not be read.") }

    fun readCommitTree(sha: String): String? {
        return when (val objectResult = read(sha, MAX_COMMIT_BYTES)) {
            is GitObjectResult.Success -> if (objectResult.type == "commit") {
                objectResult.content.toString(Charsets.UTF_8)
                    .lineSequence()
                    .takeWhile { it.isNotEmpty() }
                    .firstOrNull { it.startsWith("tree ") }
                    ?.removePrefix("tree ")
                    ?.takeIf { SHA_PATTERN.matches(it) }
            } else null
            is GitObjectResult.Unavailable -> null
        }
    }

    fun readTree(sha: String, maxEntries: Int = MAX_TREE_ENTRIES): GitObjectResultWithEntries {
        return when (val objectResult = read(sha, MAX_TREE_BYTES)) {
            is GitObjectResult.Success -> {
                if (objectResult.type != "tree") {
                    GitObjectResultWithEntries.Unavailable("Git object $sha is not a tree object.")
                } else {
                    parseTree(objectResult.content, maxEntries)
                }
            }
    ...
    private fun parseTree(content: ByteArray, maxEntries: Int): GitObjectResultWithEntries {
        ...
        while (offset < content.size && result.size < maxEntries) {
            val modeEnd = findByte(content, ' '.code.toByte(), offset)
                ?: return GitObjectResultWithEntries.Unavailable("A Git tree entry has an invalid mode field.")
            val nameEnd = findByte(content, 0, modeEnd + 1)
                ?: return GitObjectResultWithEntries.Unavailable("A Git tree entry has an unterminated name.")
            val objectIdOffset = nameEnd + 1
            if (objectIdOffset + 20 > content.size) {
                return GitObjectResultWithEntries.Unavailable("A Git tree entry is truncated.")
            }
            ...
            val objectId = content.copyOfRange(objectIdOffset, objectIdOffset + 20)
