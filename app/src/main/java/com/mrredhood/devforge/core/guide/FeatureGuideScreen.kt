        ...
        listOf(
            ...
            t("AI Mission","AI Chat presents project work as one mission: understand → plan → inspect → execute → verify → review.",
                listOf(
                    ...
                    "Internal agents, tools and provider details remain behind the workflow surface.",
                ),
            ...
            t("AI Activity Timeline","Search, reads, writes, commands, approvals, agents and verification are normalized into one compact timeline.",
                listOf(
...
t("Agent plans & activity","Chat shows the top-level engineering plan plus each agent's detailed search and execution steps.",
                ...
                "AI Chat → live agent run panel.",
                listOf("phases = sequential plan groups","agents in the same phase may run concurrently","tool steps inside an agent remain sequential and bounded")),
    ...
    GuideCategory(
        "DevForge Center",
        "A grouped control room for project health, AI inspection, testing, dependencies, backup, release, GitHub, automation, code intelligence and extensions.",
        listOf(
            t("Project Health","A single snapshot of workspace, editor, agents, approvals and recent builds.",
                listOf("Open More → DevForge Center → Project Health.","Use it as the first place to see whether the active project has pending work."),
                "More → DevForge Center → Project Health.",
                listOf("dirty = unsaved editor state","pending = waiting for approval")),
            t("Quality & Reliability","Open the unified quality dashboard for operations, workspace integrity, security review, performance budgets and release readiness.",
            ...
            t("Real extensions","Open More → Extensions for the dedicated extension workspace. Install Acode ZIPs and VS Code VSIX packages there; DevForge analyzes each package before installation and refuses unsupported runtimes, unsupported Acode APIs, or unsupported programming languages.",
                listOf(
    ...
    GuideCategory(
        ...
        listOf(
            ...
            t("Performance budget","Keep editor state within mobile-friendly bounds.",
                ...
                "More → DevForge Center → Quality & Reliability → Mobile performance budget.",
    ...
    GuideCategory(
        "Engineering Power",
        "Advanced mobile-native engineering workflows adapted from major desktop and AI coding tools without introducing local AI.",
        listOf(
            t("Engineering Power hub","Open More → AI Tools to use the unified engineering surface.",
                listOf("Mission shows Understand → Plan → Inspect → Execute → Verify → Review.","Tasks run through the existing sandboxed terminal.","Profiles keep common workflow intent grouped without duplicating Settings."),
                "More → AI Tools → Engineering Power."),
            t("Repeatable project tasks","Run bounded project commands such as tests, lint, debug/release builds and Git status.",
                listOf("Open Tasks.","Tap Run on an enabled task.","Terminal output and approval policy remain authoritative."),
                "Engineering Power → Tasks.",
                listOf("verify = test + lint","debug-build = assembleDebug","release-build = assembleRelease","unit = test","lint = lint","clean = clean","status = git status","changed = git diff --stat")),
            t("Mission Graph & Proof Package","The same AI Mission state is represented as ordered engineering phases with evidence.",
                listOf("Use Mission to see the current phase.","The proof package groups changed paths and verification results.","Build/test/lint truth still comes from Build Center receipts."),
                "Engineering Power → Mission."),
            t("Development Profiles","Switch between Mobile Safe, Android Engineering, Release and Review workflow profiles.",
                listOf("Choose one profile.","The selected profile is persisted on the device.","Profiles do not silently change security or approval policy."),
                "Engineering Power → Profiles."),
            t("Reviewer, refactor and debugger foundations","The power layer exposes structured concepts for pre-commit review, AI refactoring, critic roles, debugger state, semantic navigation, registers and branching undo.",
                listOf("Use the existing editor and AI Mission surfaces for actual mutations.","These concepts are represented as reusable internal contracts so features share one workflow instead of becoming separate agent dashboards."),
                "Engineering Power and the existing Editor/Chat surfaces."),
            t("UI journeys & Compose preview lab","Declarative journey, preview, accessibility and device-target models cover phone, tablet, foldable and multi-window validation.",
                listOf("Journeys model actions and assertions.","Preview requests model device and accessibility intent.","Runtime instrumentation remains authoritative for actual UI execution."),
                "Engineering Power → Integrations."),
            t("Remote development","Remote worker definitions provide a controller/worker model without forcing local AI or local build compute.",
                listOf("Remote endpoints are definitions, not implicit credentials.","Execution remains subject to capability and approval policy.","GitHub Actions remains the supported remote build backend today."),
                "Engineering Power → Integrations."),
            t("MCP gateway","External tool servers are represented as explicit registry entries and remain approval-aware.",
                listOf("MCP is registry-first.","A server is disabled by default.","Do not treat a registered endpoint as an approved execution path."),
                "Engineering Power → Integrations."),
            t("AI memory","Memory is split conceptually by global, workspace and mission scope and stores evidence with each entry.",
                listOf("Memory is context, not authority.","Build/test/lint receipts and project rules outrank memory.","Do not store credentials or secret values."),
                "Engineering Power core memory model."),
            t("Learned repository rules","Repeated patterns can become rule proposals rather than silent permanent behavior.",
                listOf("Proposals include evidence.","Rules require explicit acceptance before becoming trusted project guidance."),
                "Engineering Power core rule store."),
            t("Resource-aware routing","Requests are classified into fast, standard or deep work classes without enabling on-device/local AI.",
                listOf("Simple requests stay lightweight.","Build/fix/implement requests use standard routing.","Architecture, security audits and complex release work are deep-routing candidates."),
                "AI Mission routing policy."),
            t("Automations & extensions","Engineering automation blueprints and a native DevForge extension contract provide future CI/issue/dependency workflows without adopting another IDE's extension API.",
                listOf("Automation blueprints include CI failure watch, issue triage, dependency review, nightly verification and release evidence.","Extensions stay capability-scoped."),
                "Engineering Power core catalogs."),
        ),
    ),
    GuideCategory(
