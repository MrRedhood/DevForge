...
private val guideCategories = listOf(
    GuideCategory(
        ...
        listOf(
            ...
            t("Workspace file long-press actions","Long-press files and folders in the Editor drawer or Files screen to access item actions.",
                listOf(
                    "Long-press a file or folder.",
                    "Choose Rename, Delete, or Change file path.",
                    "Change file path opens the workspace-root folder picker.",
                    "Navigate to the destination folder and tap Choose this path.",
                ),
                "Editor hamburger → workspace tree; Files → workspace tree.",
                listOf("long press = actions","Choose this path = move")),
            t("Tabs","Keep multiple files open.",
                ...
                "Editor tab strip.",
                listOf("● = dirty","× = close")),
            t("Automatic workspace refresh","The Editor refreshes the visible file tree automatically after successful workspace mutations.",
                listOf("Create a file or folder from the Editor drawer.","Ask AI to create, modify, delete, or rename workspace paths.","Successful AI side effects emit a workspace change event and refresh the visible tree automatically.","The hamburger Refresh remains available as a manual fallback, but normal file/folder changes no longer require it."),
                "Editor project drawer and AI Chat.",
                listOf("auto-refresh = workspace mutation refresh","↻ in hamburger = manual fallback")),
            ...
            t("AI Mission","AI Chat shows a model-generated plan only when the model decides a multi-step task needs one.",
                listOf(
                    "Open the floating AI Chat control.",
                    "Send a coding or project-change request.",
                    "For small one-step requests, no plan card is created.",
                    ...
                    "Use Pause AI to stop the active request safely. The composer is released immediately, the active provider/tool request is cancelled, and callbacks from the paused turn cannot overwrite a newer chat turn.",
                ...
                listOf("AI's plan = model-generated action summary","plan progress = model-declared","no plan = simple task")),
            t("AI project rules and skills","Every AI Mission receives DevForge engineering policy and a matched skill profile, while project-specific rules can live in .devforge/rules/ and .devforge/skills/.",
                ...
                "Workspace .devforge/ rules and the compact AI plan/execution activity.",
                listOf("rules = project constraints","skills = reusable expertise","plan = selected skill profile")),
            t("AI Activity Timeline","Searches, reads, writes, commands, approvals and verification update live while the AI works.",
                listOf(
                    "Open AI Chat during a coding task.",
                    "Watch the chat follow the latest live activity automatically.",
                    "Expand tool entries when you need the exact tool and result detail.",
                ),
                "AI execution → live activity.",
    ...
    GuideCategory(
        ...
        listOf(
            ...
            t("Thinking mode","Ask compatible cloud models to produce a concise user-visible action summary before answering or using tools.",
                listOf(
                    "Tap the thinking/psychology icon in the Chat composer.",
                    "When enabled, DevForge asks the model for a brief <devforge_thinking> summary before the answer or tool work.",
                    "DevForge never exposes private chain-of-thought; the visible section is a concise action summary.",
                    "After completion, tap the Thinking row and its arrow to expand or collapse the saved summary.",
                    "Models that advertise native reasoning/thinking are detected and skip DevForge's extra thinking layer.",
                ),
                "AI Chat → composer → Thinking icon.",
                listOf("Thinking icon = custom thinking mode","animated Thinking… = model has not finished its action summary","arrow = expand/collapse")),
            t("Full AI tool catalog","Every registered DevForge tool is exposed to the main AI through the compatibility tool protocol; capability and approval rules still gate execution.",
                listOf("Ask AI to inspect, edit, build, test, run GitHub Actions, analyze media, use terminal operations, or perform file/folder operations.","The main AI can emit any registered <devforge_tool> envelope without relying on native provider function-calling.","Destructive operations remain approval-gated even when the tool is available."), 
                "AI Chat → execution; More → AI Tools.",
                listOf("full catalog = AI-callable","approval = safety gate")), 
            t("AI tool approvals","A tool approval pauses the exact Chat action and resumes that same action after approval.",
                listOf("Send a request that needs a protected file operation.","Open More → Approvals.","Approve the exact action.","The waiting Chat tool continues without creating a second approval for the already-approved action.","For reusable permission, add a persistent capability grant in Approval Center; deletion can now be granted within a chosen workspace/path scope.","Dismiss stops that action."),
                "More → Approvals.",
                listOf("pending = waiting for you","approved = exact request may execute","grant = reusable bounded permission","dismissed = action stops")),
            ...
            t("Chat attachments","Attach any file type from one universal Android file picker.",
                listOf(
                    "In AI Chat, tap the + button once.",
                    "The Android file picker opens with all file types available; choose images, videos, audio, text, source code, PDFs, archives, or other files.",
                    "After sending, the attachment remains visible inside the user message.",
                    "Tap a sent image or text file to view it in Chat. Video and audio files can be opened with an installed Android player.",
                    "Attachment permissions are retained for successfully sent history items so older chat attachments remain viewable.",
                ),
                "AI Chat → + attachment button.",
                listOf("+ = universal file picker","attachment card = sent file","Open = view/play")),
            ...
            t("AI Tools","Manage the tools available to the single main DevForge AI.",
                listOf(
                    ...
                    "Creation tools require a real target path: create_file/create_folder must use a concrete workspace-relative path and never '.', './', '/', or an empty path.",
                    "For creation aliases, AI may provide fileName/filename/name or folderName/name together with directory/parent/folder and DevForge combines them into the target path.",
                    "When a creation tool rejects a path, AI is instructed to correct the arguments instead of repeating the same invalid call.",
                    "Chat history is durable per provider/model/workspace session, so starting a new turn does not clear completed user or AI messages.",
                ),
                "More → AI Tools.",
    ...
    GuideCategory(
        ...
        listOf(
            ...
            t("Long-press file actions","Open the file or folder popup menu without first opening it.",
                listOf(
                    "Long-press a file or folder in the workspace tree.",
                    "Choose Rename, Delete, or Change file path.",
                    "Change file path opens a folder chooser that starts at the workspace root and shows files and folders while you navigate.",
                    "Reach the destination folder and tap Choose this path to move the selected item.",
                ),
                "Files screen → long-press an item.",
                listOf("long press = item actions","Choose this path = move destination")),
    ...
    GuideCategory(
        ...
        listOf(
            ...
            t("Open remote worktree","Treat a GitHub repository as a DevForge workspace.",
                listOf(
                    "Choose a repository.",
                    "DevForge activates or creates the matching remote workspace.",
                    "AI workspace tools read and mutate GitHub-backed paths through the GitHub repository gateway rather than treating devforge://github/... URIs as local SAF documents.",
                    "File-existence checks return a normal exists=false result when a remote path is absent.",
                    "Remote tree commits refresh the branch/base tree before retrying a transient GitHub tree 404, reducing false synchronization failures immediately after another commit.",
                    "Browse its tree.",
                ),
                "Editor hamburger → GitHub → repository.",
                listOf("remote path = repository-relative","exists=false = normal absent-path result","tree retry = transient 404 recovery")),
    ...
    GuideCategory(
        ...
        listOf(
            ...
            t("AI terminal commands","Allow AI to execute a bounded workspace command and return its result to Chat.",
                listOf("Open More → AI Tools.","Enable Run terminal command.","AI uses run_command with a Linux shell command plus optional workingDirectory and timeoutMs.","Local workspaces execute against the Android Linux shell; GitHub-backed workspaces use the API-backed virtual shell for repository inspection and common shell operations.","Review the approval request before execution when policy requires it."),
                "More → AI Tools; AI Chat and AI.",
                listOf("R2 = terminal actions are approval-gated by default","run_command = command + bounded result")),
            t("AI live GitHub Actions logs","Let AI inspect the current or latest GitHub Actions run, including jobs that are still running.",
                listOf("Open AI Chat in a GitHub-backed workspace.","AI can call github_get_actions to discover recent workflow runs.","AI can call github_get_workflow_logs without manually supplying owner, repository or runId; DevForge infers the active repository and latest run.","Running jobs expose their current job logs, so AI can inspect the output before taking the next coding, build, or GitHub action."),
                "AI Chat; More → Live GitHub Actions.",
                listOf("latest run = automatically selected","live logs = refreshed from GitHub on each tool call")),
    ...
    GuideCategory(
        ...
        listOf(
            ...
            t("Persistent capability grants","Reuse bounded permission for supported capabilities, including workspace deletion.",
                listOf("Choose a grantable capability such as Delete files.","Set an optional workspace-relative path scope.","Grant it only when you understand the risk.","A matching grant suppresses future approval prompts inside its scope.","Revoke later if needed."),
                "Approval Center → persistent grants.",
                listOf("grant = reusable permission","path scope = where it applies","Delete files = grantable")),
    ...
    GuideCategory(
        ...
        listOf(
            ...
            t("More","Secondary app functions that do not need permanent bottom-navigation space.",
                listOf("Open More.","Choose Settings, AI-driven actions, Approvals, AI Tools, Live GitHub Actions, AI & GitHub, Report a bug, or Help & guide.","More opens as a full app surface with no duplicate navigation bar."),
                "Bottom navigation → More."),
            t("Report a bug","Open the DevForge GitHub issue form so users can report problems directly to the project.",
                listOf("Open More.","Tap Report a bug.","GitHub opens a new issue page where the problem can be described and submitted."),
                "More → Report a bug."),
    ...
    GuideCategory(
        ...
        listOf(
            ...
            t("AI execution","The single main AI executes coding work through the bounded tool runtime and exposes concise live status rather than private chain-of-thought.",
                listOf("Ask DevForge AI for a coding change.","For multi-step work, the model may create a concise action plan.","Watch search, read, write, delete, command and verification activity update in the chat.","Expand an execution label to inspect its tool name and result detail."),
                "AI Chat → live plan and execution labels; More → AI & GitHub → AI Tools.",
                listOf("user = reviewer","AI = sole executor","status = concise execution summary")),
t("Plans & execution","The plan is created by the model when useful, not by a fixed app checklist.",
                listOf("Open AI Chat.","Send a coding or project-change request.","Small one-step tasks can run without a plan.","When the model creates a plan, review its concise action steps while execution continues."),
                "AI Chat → live AI plan.",
                listOf("plan = model-generated action summary","tool steps = live and bounded")),
    ...
    GuideCategory(
        ...
        listOf(
            ...
            t("File preview","Open a workspace file in the editor.",
                ...
                "Workspace tree."),
            t("Web Web Live Preview","Open a workspace file directly in the Web Web Live Preview surface with workspace-relative HTML/CSS/JavaScript assets served by the web runtime.",
                listOf(
                    "Open the file you want to preview in the Editor and tap ▶ in the upper-right editor toolbar.",
                    "When there is no current preview target, Web Live Preview opens its own workspace file chooser automatically.",
                    ...
                    "Selecting a file loads it directly into Web Live Preview; the file does not need to be opened as an editor tab.",
                    "Tapping ▶ Preview from the Editor adopts the current active editor file, while a file chosen inside Web Live Preview remains independent until the next explicit Preview launch or chooser selection.",
                    "While a selected file is being read, Web Live Preview shows a loading state; read failures are shown as an actionable error instead of a blank surface.",
                    "GitHub-backed preview reads run off the UI thread, so selecting a remote file does not block the app on a network request.",
                    "When a GitHub-backed file has an unsynced DevForge change, Web Live Preview uses the pending/current content before falling back to the repository version, so newly created or edited files can be previewed before commit.",
                    ...
                    "Use Refresh in the preview toolbar to force a manual reload.",
                    "If you switch to another editor file, the open Web Live Preview keeps its selected target until you explicitly relaunch Preview or choose another file.",
                ),
                "Editor → ▶ Preview; choose the file from the Web Live Preview chooser.",
                listOf("one preview mode","workspace chooser = independent target","automatic + manual refresh")),
    ...
    GuideCategory(
        ...
        listOf(
            t("Shell execution","Terminal commands run through the existing workspace sandbox. Local workspaces are mirrored into an isolated terminal root before execution.",
                ...
                "More → Terminal."),
            t("Expanded AI tools","The main AI catalog now exposes exactly 150 user-configurable tools.",
                listOf("Browse the complete AI tool catalog in More → AI Tools.","Models without native function-calling support use the same textual DevForge tool protocol."),
                "More → AI Tools."),
            t("150 terminal commands","The Terminal exposes a 150-command discoverability catalog covering navigation, text, files, archives, processes, Android diagnostics, networking, Git and development runtimes. The shell itself is not restricted to this list.",
                listOf("Type commands to print the full 150-command catalog.","Availability depends on the Android build/device PATH."),
            ...
            t("Core command set","The Terminal exposes a discoverable Linux-style command catalog while still allowing any command available in the Android PATH.",
                listOf(
                    ...
                    "Inspect: date, df, du, stat, readlink, realpath, basename, dirname, uname, id, whoami, env, printenv, which, ps, top, uptime, getprop and logcat.",
                    "Write/files: sed, mkdir, rmdir, touch, rm, cp, mv, ln, chmod, mktemp, dd and tee.",
                    "Archives/checksums: tar, gzip, gunzip, md5sum, sha1sum, sha256sum, sha512sum, cksum, cmp and diff.",
                    "Processes: pgrep, pidof, kill, killall, nice, renice, nohup and time.",
                    "Use the terminal command commands for the in-app catalog. Optional commands such as curl, wget, ping, nslookup, git, java, kotlinc or gradle remain available when installed.",
                ),
                "More → Terminal → type commands or help."),
