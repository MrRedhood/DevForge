...
private val guideCategories = listOf(
    ...
    GuideCategory(
        ...
        listOf(
            ...
            t("Attachment provider support","Binary uploads are provider-specific.",
                listOf("Gemini uses the Gemini Files API for binary uploads.","OpenRouter supports inline image/PDF content through its OpenAI-compatible adapter.","Providers without a binary attachment adapter reject unsupported binary sends instead of silently dropping the file."),
                "AI Chat send path.",
                listOf("attachment selected = local pending attachment","attachment sent = provider transport accepted it")),
