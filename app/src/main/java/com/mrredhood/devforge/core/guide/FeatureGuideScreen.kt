...
private val guideCategories = listOf(
        ...
        listOf(
            ...
            t("AI Activity Timeline","Search, reads, writes, commands, approvals and verification are normalized into one compact timeline.",
                listOf(
                    ...
                    "Expand the AI plan.",
                    "Review the live AI execution history.",
                ),
                "AI execution → Expand activity.",
    ...
    GuideCategory(
        "Extensions",
        "DevForge packages use the platform API and permission model so developers can build extensions, tools, agents, workflows, themes, languages, and other packages.",
        listOf(
            t("DevForge Platform API","The public v1 developer surface is broad enough for packages to extend app navigation, UI, workspaces, files, editor, languages, terminal, Git, GitHub, Build Center, artifacts, AI, agents, tools, workflows, automation, commands, settings, storage, networking, notifications and events.",
                listOf(
                    "Use the DevForge API contract instead of depending on Android or DevForge internal classes.",
                    "Read the stable TypeScript declarations in sdk/devforge/devforge.d.ts.",
                    "Use the Kotlin contract under core/extension/api when implementing native adapters.",
                    "Declare every requested permission in the package manifest.",
                    "High-risk operations remain subject to DevForge approval even when permission is declared.",
                ),
                "Developer SDK → DevForge Platform API.",
                listOf("API 1.0 = stable platform contract","permission = capability request")),
            t("DevForge package manifest","Every package declares its identity, version, package type, DevForge API compatibility, entry point, permissions, activation events and contributions.",
                listOf(
                    "Set manifestVersion to 1.",
                    "Set devforge.api to the supported API line and minimumVersion to the minimum DevForge version.",
                    "Use SemVer for package versioning.",
                    "Declare only the permissions actually required.",
                    "Validate manifests before packaging or publishing.",
                ),
                "SDK → manifest.schema.json and native manifest validator.",
                listOf("1.0.0 = package version","manifestVersion = 1")),
            t("DevForge versioning","The Platform API uses MAJOR.MINOR compatibility and stable event schemas have independent integer versions.",
                listOf(
                    "Minor API releases add backward-compatible capabilities.",
                    "Breaking API changes require a new major line.",
                    "Published package versions are immutable.",
                    "Stable APIs are deprecated before removal rather than silently disappearing.",
                ),
                "SDK → API and versioning documentation.",
                listOf("stable = production","experimental = not a production dependency")),
            t("Extension manager","Legacy Acode/VS Code compatibility remains isolated from the new DevForge-native package contract while the DevForge Marketplace is built.",
                listOf(
                    "Open More → Extensions to manage currently installed compatible packages.",
                    "The DevForge-native marketplace will use .devforge packages and the public Platform API contract.",
                ),
                "More → Extensions.",
                listOf("DevForge = native package ecosystem")),
        ),
    ),
    GuideCategory(
        "Workspaces",
