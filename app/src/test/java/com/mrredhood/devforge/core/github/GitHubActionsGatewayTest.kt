...
class GitHubActionsGatewayTest {
    ...
    @Test
    fun releaseDispatchCarriesReleaseTargetAndArtifactInputs() {
        var requested: FakeConnection? = null
        val gateway = GitHubActionsGateway(
            secretStore = FakeSecretStore("token"),
            connection = HttpConnectionFactory { url ->
                FakeConnection(
                    URL(url),
                    200,
                    """{"workflow_run_id":123,"html_url":"https://github.com/MrRedhood/DevForge/actions/runs/123"}""",
                ).also { requested = it }
            },
        )
        ...
        val result = kotlinx.coroutines.runBlocking {
            gateway.dispatch(
                ...
                "DevForge",
                BuildConfiguration(
                    githubOwner = "MrRedhood",
                    githubRepository = "DevForge",
                    workflowFile = ".github/workflows/android.yml",
                    branch = "main",
                    target = BuildTarget.ReleaseApk,
                    buildTask = BuildTarget.ReleaseApk.buildTask,
                    artifactName = BuildTarget.ReleaseApk.artifactName,
                ),
            )
        }

        assertEquals(GitHubDispatchResult.Started(123L, "https://github.com/MrRedhood/DevForge/actions/runs/123"), result)
        assertEquals("POST", requested?.requestMethod)
        assertTrue(requested?.url.toString().endsWith("/actions/workflows/android.yml/dispatches"))
        val payload = requested?.requestBody.orEmpty()
        assertTrue(payload.contains(""""target":"release_apk"""))
        assertTrue(payload.contains(""""build_artifact":"true"""))
        assertTrue(payload.contains(""""publish_artifacts":"true"""))
    }
    ...
    private class FakeConnection(
        ...
        val requestBody: String
            get() = requestOutput.toString(Charsets.UTF_8)
