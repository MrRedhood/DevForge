...
class GitHubRepositoryGatewayTest {
    ...
    @Test
    fun deletesRepositoryWithValidatedHttpsDeleteRequest() {
        var openedConnection: FakeConnection? = null
        val gateway = GitHubRepositoryGateway(
            secretStore = FakeSecretStore("token"),
            connection = HttpConnectionFactory {
                FakeConnection(URL(it), 204, "").also { openedConnection = it }
            },
        )

        val result = gateway.deleteRepository("MrRedhood", "DevForge")

        assertTrue(result.isSuccess)
        assertEquals("DELETE", openedConnection?.requestMethod)
        assertEquals("Bearer token", openedConnection?.requestProperty("Authorization"))
        assertTrue(openedConnection?.url.toString().endsWith("/repos/MrRedhood/DevForge"))
    }
