...
- [x] No production navigation behavior was changed; this repair aligns regression coverage with the existing UI and policy.
- [ ] Fresh Android UI Tests validation is required for this repair commit.
...
### 2026-09-23 — First-party DevForge Marketplace
- [x] Removed the in-app Acode and VS Code catalog discovery paths and replaced the Discover surface with the first-party DevForge Marketplace.
- [x] Added Marketplace categories for extensions, AI agents/tools, workflows, automations, themes, icon packs, languages, templates, tool packs, and projects.
- [x] Added configurable HTTPS Marketplace server settings so the Android app can point to the deployed first-party API without requiring an APK source change.
- [x] Changed remote installation to accept only DevForge Marketplace packages and validate downloads against the configured Marketplace host.
- [x] Added DevForge-native manifest analysis for .devforge packages and native package compatibility classification.
- [x] Updated More, navigation, Help & guide, README, and UI regression coverage from Extensions/Acode/VS Code catalogs to Marketplace.
- [x] Added a Cloudflare Workers + D1 + R2 Marketplace backend foundation under marketplace/ with catalog, immutable releases, download, and bootstrap publisher endpoints.
- [x] Added Marketplace deployment documentation and a seed manifest for the initial package publishing pipeline.
...
### 2026-09-23 — AI multi-file execution, plan progress, automatic workspace refresh, and repository delete layout follow-up
- [x] Fixed the validated GitHub repository Delete repository action layout by stacking Edit/Delete controls at full width so narrow Android screens cannot wrap the destructive label character-by-character.
- [x] Added a workspace change event bus and automatic visible-tree refresh for successful AI side-effecting tool executions, including file/folder creation, deletion, rename, patches, and terminal-side mutations.
- [x] Removed the broken Editor top-right Refresh button; the hamburger Refresh remains only as a manual fallback while normal workspace mutations refresh automatically.
- [x] Increased the single-AI tool budget to 48 bounded calls/steps to support larger multi-file and multi-folder tasks without stopping prematurely.
- [x] Added cooperative cancellation checks and bounded 90-second model turns / 120-second tool executions so a stuck model/tool cannot hold the Chat task indefinitely.
- [x] Replaced the ambiguous AI stop square with an explicit Pause AI control; pausing cancels the active request safely and restores the request to the composer for a later retry from the current workspace state.
...
- [x] Added Android UI coverage for the no-target chooser state and actionable workspace-unavailable message.
